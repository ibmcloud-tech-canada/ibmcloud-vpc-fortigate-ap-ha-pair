...
resource "ibm_is_floating_ip" "publicip3" {
  depends_on = [
    ibm_is_instance.fgt2
  ]
  name   = "${var.CLUSTER_NAME}-publicip-fgt2-${random_string.random_suffix.result}"
  resource_group = var.RESOURCE_GROUP
...
resource "ibm_is_instance" "fgt1" {
  ...
  primary_network_interface {
    ...
    primary_ip {
      address     = var.FGT1_STATIC_IP_PORT1
      auto_delete = true
    }
  }
  ...
  network_interfaces {
    ...
    primary_ip {
      address     = var.FGT1_STATIC_IP_PORT2
      auto_delete = true
    }
  }
  ...
  network_interfaces {
    ...
    allow_ip_spoofing = false
    primary_ip {
      address     = var.FGT1_STATIC_IP_PORT3
      auto_delete = true
    }
  }
  network_interfaces {
    ...
    allow_ip_spoofing = false
    primary_ip {
      address     = var.FGT1_STATIC_IP_PORT4
      auto_delete = true
    }
  }
...
resource "ibm_is_instance" "fgt2" {
  ...
  primary_network_interface {
    ...
    allow_ip_spoofing = false
      primary_ip {
      address     = var.FGT2_STATIC_IP_PORT1
      auto_delete = true
    }
  }
  ...
  network_interfaces {
    ...
    allow_ip_spoofing = true
    primary_ip {
      address     = var.FGT2_STATIC_IP_PORT2
      auto_delete = true
    }
  }
  network_interfaces {
    ...
    allow_ip_spoofing = false
    primary_ip {
      address     = var.FGT2_STATIC_IP_PORT3
      auto_delete = true
    }
  }
  network_interfaces {
    ...
    allow_ip_spoofing = false
    primary_ip {
      address     = var.FGT2_STATIC_IP_PORT4
      auto_delete = false
    }
  }
...
data "template_file" "userdata_active" {
  template = file(join("/",["${path.module}","user_data_active.conf"]))
  ...
  vars = {
    ...
    netmask                  = var.NETMASK
    ibm_api_key              = ibm_iam_service_api_key.fortigate-api-key.apikey
...
data "template_file" "userdata_passive" {
  template = file(join("/",["${path.module}","user_data_passive.conf"]))
